...
class MealsList extends React.Component {
  ...
  sortByProperty = (items, property) => items.sort((a, b) => {
  ...
  handleCuisineChange = (value) => {
    ...
    meals.forEach((meal) => {
      if (meal.cuisineType === value) queryResult.push(meal);
    });
    ...
    if (this.props.isLoading) {
      return (
        <div className="container pointer-events--none">
          <div className="mdl-spinner mdl-spinner--single-color mdl-js-spinner is-active pointer-events--none" />
        </div>
      );
    }
    return (
      <div>
        ...
        <div className="container">
          <p>
            <span className="mdl-number">{this.state.queryResult.length}</span>
            {' '}
            recipes shown.
          </p>
        </div>
        ...
        <div className="container-flow">
          ...
          { (this.state.queryResult.length == 0)
            ? (
              <p>
                No
                {this.state.viewableCuisineTypes[0]}
                {' '}
                results found, sadly!
              </p>
            )
